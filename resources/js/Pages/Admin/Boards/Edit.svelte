<script>
  ...
  function submit() {
    $form.put(`/admin/boards/${board.id}`);
  }
</script>

<svelte:head>
  <title>{t.admin.boards.edit}: {board.name}</title>
</svelte:head>

<Layout>
  <div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">{t.admin.boards.edit}: {board.name}</h1>
